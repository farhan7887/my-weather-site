...
export default function HomeClient({ campusImageUrl }: HomeClientProps) {
  ...
  return (
    <main
      className={`relative min-h-screen overflow-hidden bg-gradient-to-br ${background} text-white transition-all duration-1000`}
    >
      {/* =========================================================
      ...
      {campusImageUrl && (
        <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
          {/* Blurred campus image */}
          <div
            className="absolute inset-0 scale-105 bg-cover bg-center blur-sm"
            style={{
              backgroundImage: `url("${campusImageUrl}")`,
            }}
          />

          {/* Dark premium readability overlay */}
      ...
      {/* Weather gradient overlay.
          Stays above the image so weather-based colors still
          influence the visual atmosphere. */}
      <div
        className={`pointer-events-none fixed inset-0 z-[1] bg-gradient-to-br ${background} ${
          campusImageUrl ? "opacity-35" : "opacity-100"
        } transition-all duration-1000`}
      />

      {/* Additional readability layer */}
      <div className="pointer-events-none fixed inset-0 z-[2] bg-gradient-to-b from-black/10 via-transparent to-black/30" />
      ...
      <div className="relative z-10 mx-auto min-h-screen max-w-7xl px-5 py-8 sm:px-8 lg:px-10">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/25 bg-white/10 backdrop-blur-xl">
          ...
          <div className="hidden items-center gap-2 rounded-full border border-white/25 bg-white/10 px-4 py-2 text-sm text-white/80 shadow-lg shadow-black/10 backdrop-blur-xl sm:flex">
        ...
        <section className="mx-auto flex max-w-4xl flex-col items-center pb-12 pt-20 text-center sm:pt-28">
          ...
          >
            <div className="group flex items-center rounded-3xl border border-white/20 bg-white/10 p-2 shadow-2xl shadow-black/40 backdrop-blur-2xl transition-all duration-300 focus-within:border-white/40 focus-within:bg-white/15">
          ...
          {!weather && !loading && <LocationWeatherCard />}
        </section>
                    ...
                    <div className="mt-8 flex items-center gap-5">
                      <motion.div
                        animate={{
                          y: [0, -8, 0],
                          rotate:
                            weatherInfo.type === "sunny" ? [0, 8, -8, 0] : 0,
                        }}
                        transition={{
                          duration: 4,
                          repeat: Infinity,
        ...
        <footer className="border-t border-white/10 py-6 text-center text-sm text-white/40">
          {campusImageUrl && (
            <p className="mb-2 text-xs text-white/20">
              Background: COMSATS University Islamabad
            </p>
          )}

          <p>
            Made by farri student of comsats university Islamabad. All
            weather data is provided securely All rights reserved. &copy;{" "}
